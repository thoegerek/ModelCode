...
text(.025,.9,'$\delta_1\uparrow$')
text(.025,.75,'$\varepsilon\uparrow$')
%
nexttile
...
text(.025,.9,'$\varepsilon\uparrow$')
text(.014,.75,'$\lambda\downarrow$')
...
xlim([0 .1])
ylim([ymin ymax])
xlabel('Learning efficiency ($\varepsilon$)')
...
text(.03,ymax,[strcat('$\delta_1$ = ',num2str(pfb.data{2}));strcat("$\mu$ = ",num2str(pfb.data{5}))],'HorizontalAlignment', 'right','Verticalalignment','top');
ylim([0 ymax])
xlim([0 .03])
xlabel('Learning efficiency ($\varepsilon$)')
...
text(.00085,ymax,[strcat('$\varepsilon$ = ',num2str(pfbay.data{3}));strcat("$\mu$ = ",num2str(pfbay.data{5}))],'HorizontalAlignment', 'right','Verticalalignment','top');
ylim([0 ymax])
xlim([.0001 .001])
xlabel('Remote CNDD ($\delta_1$)')
...
xlabel('Learning efficiency log (log$_{10}(\varepsilon)$)')
ylabel('Mortality rate ($\mu$)')
set(gca,'ydir','normal')
cb = colorbar;
yl = ylabel(cb,'Remote CNDD log (log$_{10}(\delta_1)$)','FontSize',14,'Rotation',270,'interpreter','latex');
...
annotation('textbox',[.025 .025 .1 .1],'string','Remote CNDD ($\delta_1$)','edgecolor','none');
annotation('textbox',[.625 .025 .1 .1],'string','Sociality ($\sigma$)','edgecolor','none');
...
plot([f_bif.ub 1],[0 0]+8,'linewidth',.5,'color',[0    0.4470    0.7410])
plot([f_bif.ub 1],[0 0]+4,'linewidth',.5,'color',C.bireg)
plot([f_bif.ub 1],[0 0],'linewidth',.5,'color',[.8 .6 0])

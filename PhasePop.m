...
Xspan = [0 100];%1.5*max(steqx)];
Yspan = [0 100];%1.5*max(steqy)];
...
if length(Sepind) == 2
    [~,A2] = ode15s(@myModel,Trange,[Saddle(1),Saddle(2)-.001],[],ax,ay,b,c,d,mu,tau);
    [~,A3] = ode15s(@myModel,Trange,[Saddle(1),Saddle(2)+.0001],[],ax,ay,b,c,d,mu,tau);
else
    A2 = [nan nan];
    A3 = [nan nan];
end
